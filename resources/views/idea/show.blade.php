    ...
    <div class="mx-auto max-w-3xl space-y-8 py-10">
        ...
        <div class="flex items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                ...
                <form method="POST" action="{{ route('ideas.destroy', $idea) }}">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="rounded-md px-4 py-2 text-sm font-medium text-red-600 hover:bg-red-50">
                        Delete
                    </button>
                </form>
            </div>
        ...
        <header>
            ...
            <div class="mt-4 flex flex-wrap items-center gap-3">
                <span class="inline-block rounded-full border border-primary/20 bg-primary/10 px-2 py-1 text-xs font-medium text-primary">
                    {{ $idea->status->label() }}
                </span>
            ...
            <section class="space-y-4">
                ...
                <div class="space-y-3">
                    @foreach ($idea->links as $link)
                        ...
                        <x-card>
                            <a href="{{ $link }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 text-primary hover:underline">
