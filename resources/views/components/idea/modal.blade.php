...
<x-modal :name="$name" :title="$idea ? 'Edit Idea' : 'New Idea'">
    <form
        ...
        x-data="{ 
            ...
            newStep: '', 
            steps: @js(old('steps', $idea?->steps ? $idea->steps->map(fn($s) => ['id' => $s->id, 'description' => $s->description, 'completed' => (bool) $s->completed])->all() : []))
        }"
    >
        @csrf
        @if ($idea)
        ...
        <fieldset class="space-y-2">
            ...
            <div class="flex gap-2">
                <input
                    ...
                    x-on:keydown.enter.prevent="if (newStep.trim()) { steps.push({id: null, description: newStep.trim(), completed: false}); newStep = ''; }"
                ...
                <button
                    type="button"
                    class="btn shrink-0"
                    x-on:click="if (newStep.trim()) { steps.push({id: null, description: newStep.trim(), completed: false}); newStep = ''; }"
